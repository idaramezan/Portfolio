...
const words = {
  en: {
    ...
    mailIntro: "A small collection of things I made for this month, sent only to the people who choose to keep a piece of it.", mailAdd: "Add this edition to bag", inside: "Inside this edition", once: "Made once, then gone.", collectible: "The pieces inside each Mail Club are created for that edition only. I do not plan to repeat them in future months or add them to the regular shop. Mail Club is my way of keeping a smaller circle around the work. Little pieces of what I am making, thinking about and living through, shared with the people who choose to keep them.", closed: "This edition has closed.", until: "Available only until",
    contents: [["Exclusive print", "A print created only for this Mail Club. It will not be released in the shop later."], ["Personal letter", "A letter from me with thoughts and stories I do not share publicly online."], ["Sticker sheet", "A small sticker sheet made for this month's edition."], ["Habit tracker", "An illustrated page to use throughout the month."], ["Bookmark", "A bookmark made for this edition."], ["One surprise", "One piece stays secret until your envelope arrives."]],
  ...
  tr: {
    ...
    mailIntro: "Bu ay için hazırladığım küçük bir koleksiyon, ondan bir parça saklamayı seçen insanlara gönderiliyor.", mailAdd: "Bu edisyonu sepete ekle", inside: "Bu edisyonun içinde", once: "Bir kez hazırlanır, sonra biter.", collectible: "Her Mail Club'ın içindeki parçalar yalnızca o aya özel hazırlanır. Gelecek aylarda tekrar etmeyi veya normal mağazaya eklemeyi planlamıyorum. Mail Club, yaptıklarımı, düşündüklerimi ve hayatımdan küçük parçaları daha küçük bir çevreyle paylaşma şeklim. Onları saklamak isteyen insanlara ait küçük parçalar.", closed: "Bu edisyon sona erdi.", until: "Yalnızca şu tarihe kadar mevcut:",
    contents: [["Özel baskı", "Yalnızca bu Mail Club için hazırlanan ve daha sonra mağazada yayınlanmayacak bir baskı."], ["Kişisel mektup", "Herkese açık olarak paylaşmadığım düşünce ve hikâyelerden bir mektup."], ["Sticker sayfası", "Bu ayın edisyonu için hazırlanan küçük bir sticker sayfası."], ["Alışkanlık takip çizelgesi", "Ay boyunca kullanabileceğin illüstrasyonlu bir sayfa."], ["Kitap ayracı", "Bu edisyon için hazırlanan bir kitap ayracı."], ["Bir sürpriz", "Zarfın gelene kadar bir parça gizli kalır."]],
...
export default function HomeCommerce() {
  ...
  const readyPalettes = newest(settings.readyMadePalettes.filter((p) => p.status === "available" && p.stock > 0));
  const paletteAvailable = settings.paletteSettings.enabled && settings.paletteSettings.types.some((type) => type.enabled);
  ...
  return <>
    ...
    {local && settings.paletteSettings.enabled && <section className="commerce-feature section-shell">
      ...
      <div className="commerce-feature__panel"><p className="eyebrow">{t.project}</p><h2>{t.palette}</h2><p>{t.paletteBody}</p><p>{t.live}</p><strong>{t.custom}</strong><span>{t.free}</span>{paletteAvailable ? <Link href="/shop/palettes/custom" onClick={() => trackAnalytics("custom_palette_started", { metadata: { locale, shippingCountry: "TR" } })}>{t.design} →</Link> : <p>{locale === "tr" ? "Kişiye özel paletlere kısa bir ara verildi." : "Custom palettes are taking a short pause."}</p>}</div>
    </section>}
    {local && readyPalettes.length > 0 && <ProductSection eyebrow={t.available} title={t.ready} body={t.readyBody}>
      {readyPalettes.map((p) => <article className="ready-palette" key={p.id}><img src={p.imageUrl} alt={p.altText || p.name} loading="lazy"/><div><p className="eyebrow">{t.available}</p><h3>{locale === "tr" && p.nameTr ? p.nameTr : p.name}</h3><p>{p.colors || (locale === "tr" && p.descriptionTr ? p.descriptionTr : p.description)}</p>{p.note && <p>{p.note}</p>}<strong><Money baseAmountUsdCents={p.priceMinor} canonicalCurrency="TRY" /></strong><button onClick={() => add({ id: `ready-palette-${p.id}`, productId: p.id, kind: "ready-palette", title: p.name, imageUrl: p.imageUrl, priceUsdCents: p.priceMinor, canonicalCurrency: "TRY", canonicalPriceMinor: p.priceMinor, quantity: 1 }, p.stock)}>{t.add}</button></div></article>)}
    </ProductSection>}
    {local && currentMail && <section className="commerce-feature commerce-feature--mail section-shell">
      <img src={currentMail.coverImage} alt={locale === "tr" && currentMail.altTextTr ? currentMail.altTextTr : currentMail.altText} width="1355" height="1824" loading="lazy" />
      <MailClubPanel edition={currentMail} locale={locale} text={t} onAdd={() => add({ id: `mail-club-${currentMail.id}`, productId: currentMail.id, kind: "mail-club", title: currentMail.title, imageUrl: currentMail.coverImage, priceUsdCents: currentMail.priceMinor, canonicalCurrency: "TRY", canonicalPriceMinor: currentMail.priceMinor, quantity: 1, metadata: { editionTitle: currentMail.title, editionMonth: currentMail.monthYear } }, currentMail.stock)} />
    </section>}
...
function MailClubPanel({ edition, locale, text, onAdd }: { edition: ReturnType<typeof useShopSettings>["mailClubEditions"][number]; locale: "en" | "tr"; text: typeof words.en | typeof words.tr; onAdd: () => void }) {
  const [now, setNow] = useState(Date.now());
  useEffect(() => { if (!edition.availabilityEnd) return; const timer = window.setInterval(() => setNow(Date.now()), 30_000); return () => window.clearInterval(timer); }, [edition.availabilityEnd]);
  const start = edition.availabilityStart ? Date.parse(edition.availabilityStart) : null; const end = edition.availabilityEnd ? Date.parse(edition.availabilityEnd) : null;
  const open = edition.status === "published" && edition.enabled && edition.stock > 0 && (!start || now >= start) && (!end || now < end);
  const remaining = end ? Math.max(0, end-now) : 0; const days = Math.floor(remaining/86400000); const hours = Math.floor((remaining%86400000)/3600000); const endLabel = end ? new Intl.DateTimeFormat(locale === "tr" ? "tr-TR" : "en-GB", { day: "numeric", month: "long" }).format(end) : "";
  return <div className="commerce-feature__panel"><h2>{locale === "tr" && edition.titleTr ? edition.titleTr : edition.title}</h2><p>{locale === "tr" && edition.descriptionTr ? edition.descriptionTr : text.mailIntro}</p><h3>{text.inside}</h3><div className="mail-club-contents">{text.contents.map(([title,body]) => <div key={title}><strong>{title}</strong><p>{body}</p></div>)}</div><div className="mail-club-note"><strong>{text.once}</strong><p>{text.collectible}</p></div>{end && remaining > 0 && <div className="mail-club-countdown"><span>{text.until} {endLabel}</span><strong>{days} {locale === "tr" ? "gün" : "days"} · {hours} {locale === "tr" ? "saat" : "hours"}</strong></div>}<strong><Money baseAmountUsdCents={edition.priceMinor} canonicalCurrency="TRY" /></strong><span>{text.free}</span>{open ? <button onClick={onAdd}>{text.mailAdd}</button> : <span>{end && now >= end ? text.closed : text.sold}</span>}</div>;
}
...
function ProductSection({ eyebrow, title, body, cta, href, children }: { eyebrow: string; title: string; body?: string; cta?: string; href?: string; children: React.ReactNode }) {
  return <section className="commerce-section section-shell"><header><div><p className="eyebrow">{eyebrow}</p><h2>{title}</h2></div>{body && <p>{body}</p>}</header><div className="commerce-grid">{children}</div>{cta && href && <Link className="home-green-rule-link" href={href}>{cta} →</Link>}</section>;
}
