...
import { useListEvents, getListEventsQueryKey, Event, useSignupForEvent } from "@workspace/api-client-react";
...
function EventCard({ event, index }: { event: Event, index: number }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    signup.mutate({ id: event.id, data: { name, email } }, {
      onSuccess: () => {
        setSubmitted(true);
      }
    });
  ...
  return (
    <div className={cn("bg-paper border-2 border-ink p-6 md:p-10 relative overflow-hidden", edgeClass)}>
      {/* Tape decoration */}
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        <div className="flex-1">
          <div className="mb-4">
            <span className="font-hand text-3xl md:text-4xl text-coral block mb-2">{format(date, 'MMMM do, yyyy')}</span>
            <span className="font-sans text-ink/70 font-medium uppercase tracking-widest text-sm">{format(date, 'h:mm a')} • {event.location}</span>
          </div>
          
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-ink mb-4">{event.title}</h2>
          ...
          {showForm && !submitted && (
            <form onSubmit={handleSubmit} className="bg-ochre/10 p-6 torn-edge-3 animate-in fade-in slide-in-from-top-4">
              <h4 className="font-serif text-xl font-bold mb-4 text-ink">Save your spot</h4>
          ...
          {submitted && (
            <div className="bg-green/10 border border-green p-6 torn-edge text-center animate-in zoom-in-95">
              <p className="font-hand text-3xl text-green mb-2">You're on the list!</p>
              <p className="font-sans text-ink">We've saved your spot for {event.title}. See you there!</p>
            </div>
          )}
        </div>
...
export default function Events() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-12 md:py-24">
      <div className="mb-16">
        <h1 className="text-5xl md:text-7xl font-serif text-ink mb-6">Upcoming Events</h1>
      ...
      {isLoading ? (
        ...
        <div className="w-full py-32 flex flex-col items-center justify-center text-center bg-card border border-ink/10 torn-edge-2 relative">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-4 font-hand text-4xl text-ink rotate-[-5deg]">Empty</div>
          <h3 className="font-serif text-3xl text-ink mb-4 mt-6">No upcoming events</h3>
          <p className="text-muted-foreground font-sans text-lg max-w-md mx-auto">
            I'm currently busy in the studio working on new pieces. Join the newsletter to hear when the next show is announced.
          </p>
        </div>
      )}
    </div>
