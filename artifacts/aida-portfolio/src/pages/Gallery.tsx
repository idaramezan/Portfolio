  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-12 md:py-24">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
        <div>
          <h1 className="text-5xl md:text-7xl font-serif text-ink mb-4">Gallery</h1>
          <p className="text-xl text-muted-foreground font-sans max-w-xl">
            A complete archive of selected oil pastel works.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 md:gap-4">
          {CATEGORIES.map((cat) => (
            <button
              ...
              style={{
                clipPath: cat === "All" ? "polygon(2% 0, 100% 2%, 98% 100%, 0 98%)" : 
                          cat === "Animals" ? "polygon(0 2%, 98% 0, 100% 98%, 2% 100%)" : 
      ...
      {isLoading ? (
        ...
        <div className="w-full py-32 flex flex-col items-center justify-center text-center bg-ink/5 torn-edge-2">
          <h3 className="font-serif text-3xl text-ink mb-4">No artworks found</h3>
          <p className="text-muted-foreground font-sans text-lg">Try selecting a different category.</p>
          <button 
            onClick={() => setActiveCategory("All")}
            className="mt-8 font-serif text-coral link-underline text-xl"
          >
            Clear filters
          </button>
        </div>
