...
export default function PrintDetail({ market: _market }: { market: Market }) {
  ...
  useEffect(() => {
    ...
    setFraming(
      product.printOptions?.framing.unframedAvailable ? "unframed" : "framed",
    );
    setQuantity(1);
    setAdded(false);
    setPurchaseError("");
  ...
  const addConfiguredPrint = () => {
    if (!canAdd || !size || !product.printOptions) {
      setPurchaseError(
        locale === "tr"
          ? "Bu baskı bu seçeneklerle sipariş edilemiyor."
          : "This print cannot be ordered with these options.",
      );
      return;
    }
    ...
    if (!result.ok) {
      setPurchaseError(
        result.reason ||
          (locale === "tr"
            ? "Baskı sepete eklenemedi."
            : "The print could not be added."),
      );
      return;
    }
    setPurchaseError("");
    ...
    trackAnalytics("local_purchase_selected", {
      metadata: { productId: product.id, framing, sizeId: size.id, quantity },
    });
    toast({
              ...
              {sold ? (
                ...
                <div className="print-detail-purchase">
                  {availableSizes.length > 1 ? (
                    <fieldset>
                      ...
                      <div className="mt-3 grid gap-2">
                        {availableSizes.map((option) => {
                          ...
                          const optionSale = calculateProductSale(
                            product.priceUsdCents,
                            product.sale,
                          );
                          const optionPrice =
                            optionSale.finalPriceMinor +
                            option.additionalPriceUsdCents;
                          return (
                            <label
                              key={option.id}
                              className={`flex min-h-14 cursor-pointer items-center gap-3 border p-3 transition-colors focus-within:ring-2 focus-within:ring-coral ${selectedSize ? "border-green bg-green/5" : "border-ink/15"}`}
                            >
                              <input
                                type="radio"
                                name="detail-print-size"
                              ...
                              <Money
                                baseAmountUsdCents={optionPrice}
                                canonicalCurrency="TRY"
                    ...
                    >
                      {locale === "tr"
                        ? "Şu anda kullanılabilir baskı boyutu yok."
                  ...
                  {size && (
                    <section className="mt-6 border-t border-ink/10 pt-6">
                      <p className="eyebrow">
                        {locale === "tr" ? "FİYAT" : "PRICE"}
                      </p>
                      {baseSale.status === "active" ? (
                        <div className="mt-2">
                          <Money
                            baseAmountUsdCents={originalPrintUnit}
                            canonicalCurrency="TRY"
                            className="block text-sm text-ink/45 line-through"
                          />
                          <Money
                            baseAmountUsdCents={discountedPrintUnit}
                            canonicalCurrency="TRY"
                            className="mt-1 block font-sans text-3xl font-bold text-green"
                          />
                          <p className="mt-1 text-sm font-semibold text-coral">
                            {baseSale.percentage}%{" "}
                            {locale === "tr" ? "indirim" : "off"} ·{" "}
                            {locale === "tr" ? "Tasarruf" : "Save"}{" "}
                            <Money
                              baseAmountUsdCents={baseSale.discountAmountMinor}
                              canonicalCurrency="TRY"
                            />
                          </p>
                        </div>
                        ...
                        <Money
                          baseAmountUsdCents={discountedPrintUnit}
                          canonicalCurrency="TRY"
                          className="mt-2 block font-sans text-3xl font-bold"
                        />
                      )}
                    </section>
                  )}
                  ...
                  {size &&
                    ...
                    product.printOptions.framing.framedAvailable && (
                      <fieldset className="mt-6 border-t border-ink/10 pt-6">
                        <legend className="eyebrow">
                          {locale === "tr" ? "ÇERÇEVE" : "FRAMING"}
                        </legend>
                        <label
                          className={`mt-3 flex min-h-16 cursor-pointer items-center gap-3 border p-4 transition-colors focus-within:ring-2 focus-within:ring-coral ${framing === "framed" ? "border-green bg-green/5" : "border-ink/15"}`}
                        >
                          <input
                            type="checkbox"
                            className="sr-only"
                            ...
                            onChange={(event) =>
                              setFraming(
                                event.target.checked ? "framed" : "unframed",
                              )
                            }
                            aria-describedby="detail-frame-description"
                          />
                          <span
                            className={`grid h-6 w-6 shrink-0 place-items-center border ${framing === "framed" ? "border-green bg-green text-paper" : "border-ink/30"}`}
                            aria-hidden="true"
                          >
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            <span
                              id="detail-frame-description"
                              className="mt-1 block text-xs text-ink/50"
                            >
                              {locale === "tr"
                                ? "Teslimattan önce siyah çerçeveyle hazırlanır."
                                : "Prepared in a black frame before delivery."}
                            </span>
                          </span>
                  ...
                  <section className="mt-6 border-t border-ink/10 pt-6">
                    ...
                    <div className="mt-3 inline-flex items-center border border-ink/15">
                      ...
                      <span
                        className="grid h-11 min-w-12 place-items-center border-x border-ink/15 font-semibold"
                        aria-live="polite"
                      >
                        {quantity}
                      </span>
                  ...
                  >
                    ...
                    <div className="mt-4 space-y-2 text-sm">
                      <p className="flex justify-between gap-4">
                        <span>{locale === "tr" ? "Baskı" : "Print"}</span>
                        <Money
                          baseAmountUsdCents={discountedPrintUnit * quantity}
                          canonicalCurrency="TRY"
                        />
                      </p>
                      {framing === "framed" && (
                        <p className="flex justify-between gap-4">
                          ...
                          <span>
                            +
                            <Money
                              baseAmountUsdCents={frameAdditional * quantity}
                              canonicalCurrency="TRY"
                            />
                          </span>
                        </p>
                    ...
                    {totalSavings > 0 && (
                      <div className="mt-4 border-l-2 border-green pl-3 text-green">
                        <strong>
                        ...
                        <p className="mt-1 text-xs text-ink/55">
                          {[
                            productSavings
                              ? `${locale === "tr" ? "Ürün indirimi" : "Product discount"} ${(productSavings / 100).toLocaleString(locale === "tr" ? "tr-TR" : "en-US")} TL`
                              : "",
                            shippingSavings
                              ? `${locale === "tr" ? "Kargo" : "Delivery"} 50 TL`
                              : "",
                          ]
                            .filter(Boolean)
                            .join(" · ")}
                  ...
                  <button
                    type="button"
                    className={`button-primary mt-6 w-full ${added ? "!bg-green !text-paper" : ""}`}
                    disabled={!canAdd || added}
                    onClick={addConfiguredPrint}
                  >
                    {added ? (
                      <>
                        <Check size={17} />
                        {locale === "tr" ? "Sepete eklendi" : "Added to basket"}
                      </>
                    ) : locale === "tr" ? (
                      "Sepete ekle"
                    ) : (
                  ...
                  <p className="mt-4 flex items-center gap-2 text-sm text-ink/60">
                    ...
                    {locale === "tr"
                      ? "Türkiye teslimatı otomatik hesaplanır."
                      : "Türkiye delivery is calculated automatically."}
                  </p>
                </div>
