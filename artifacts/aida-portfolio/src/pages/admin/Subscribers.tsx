    ...
    <AdminLayout title="Newsletter subscribers">
      <section className="grid gap-4 sm:grid-cols-2">
        <div className="border border-ink/10 bg-paper p-5">
          ...
          <p className="mt-3 font-serif text-4xl">
            {
              subscribers.filter((subscriber) => !subscriber.unsubscribedAt)
                .length
            }
          </p>
        </div>
        <div className="border border-ink/10 bg-paper p-5">
          ...
          <p className="mt-3 font-serif text-4xl">
            {
              subscribers.filter((subscriber) => subscriber.welcomeEmailSentAt)
                .length
            }
          </p>
        </div>
      ...
      <section className="mt-6 border border-ink/10 bg-paper">
        ...
        {loading ? (
          ...
          <p role="alert" className="p-6 text-sm font-semibold text-coral">
            {error}
          </p>
        ) : filtered.length === 0 ? (
                ...
                {filtered.map((subscriber) => (
                  <tr key={subscriber.id}>
                    <td className="px-4 py-4">
                      {subscriber.name && (
                        <strong className="block">{subscriber.name}</strong>
                      )}
                      <a
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`inline-flex px-2.5 py-1 text-xs font-bold uppercase tracking-wider ${subscriber.welcomeEmailSentAt ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-900"}`}
                      >
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`inline-flex px-2.5 py-1 text-xs font-bold uppercase tracking-wider ${subscriber.unsubscribedAt ? "bg-stone-200 text-stone-700" : "bg-green-100 text-green-800"}`}
                      >
