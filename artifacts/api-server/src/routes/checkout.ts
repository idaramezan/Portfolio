...
const receiptDir = path.resolve(
  process.env.PRIVATE_RECEIPTS_DIR ||
    path.join(process.env.DATA_DIR || "data", "private-receipts"),
);
const upload = multer({
...
async function ensureSchema() {
  ...
  await pool.query(
    `ALTER TABLE event_banner_config ADD COLUMN IF NOT EXISTS booking_open_at TIMESTAMPTZ, ADD COLUMN IF NOT EXISTS booking_close_at TIMESTAMPTZ`,
  );
}

async function calculate(body: any) {
  const market =
    body.market === "international_original"
      ? "international_original"
      : "turkiye";
  const requested = Array.isArray(body.items) ? body.items : [];
  if (!requested.length || requested.length > 30)
    throw new Error("Your basket is empty or invalid.");
  ...
  for (const input of requested) {
    ...
    if (!Number.isInteger(quantity) || quantity < 1 || quantity > 20)
      throw new Error("A basket quantity is invalid.");
    const kind =
      input.kind === "original"
        ? "original"
        : input.kind === "aceo"
    ...
    if (
      ...
      input.selectedOptions?.sizeId
    ) {
      const size = product.printOptions.sizes?.find(
        (x: any) => x.id === input.selectedOptions.sizeId && x.available,
      );
      ...
      if (input.selectedOptions.framing === "framed") {
        if (!product.printOptions.framing?.framedAvailable)
          throw new Error("Framing is unavailable.");
        unit += Number(
...
publicRouter.get("/bank/:currency", limited, async (req, res) => {
  try {
    await ensureSchema();
    const currency = req.params.currency === "USD" ? "USD" : "TRY";
    const result = await pool.query(
      "SELECT account_holder, bank_name, iban, swift_bic, branch_info, instructions FROM bank_transfer_settings WHERE currency=$1 AND enabled=TRUE",
      [currency],
    );
    if (!result.rows[0])
      return res
        .status(503)
        .json({ error: `${currency} bank transfer is not configured yet.` });
    return res.json(result.rows[0]);
  } catch {
    return res
      .status(500)
      .json({ error: "Bank instructions could not be loaded." });
  }
});
publicRouter.post(
  "/orders",
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        body.market === "turkiye" &&
        (countryCode !== "TR" ||
          !/^\d{5}$/.test(clean(body.postalCode, 5)) ||
...
publicRouter.get("/orders/:number", limited, async (req, res) => {
  try {
    await ensureSchema();
    const result = await pool.query(
      "SELECT order_number,status,currency,grand_total_minor,customer_email,country_name,city,created_at FROM checkout_orders WHERE order_number=$1",
      [req.params.number],
    );
    if (!result.rows[0])
      return res.status(404).json({ error: "Order not found" });
    const row = result.rows[0];
    row.customer_email = row.customer_email.replace(
      /^(.{1,2}).*(@.*)$/,
      "$1•••$2",
    );
    return res.json(row);
...
publicRouter.get("/events/:id", limited, async (req, res) => {
  await ensureSchema();
  const result = await pool.query(
    "SELECT id,title_en,title_tr,event_start_at,timezone,location_text_en,location_text_tr,audience,total_capacity,status,enabled,display_end_at FROM event_banner_config WHERE id=$1",
    [req.params.id],
  );
  if (!result.rows[0])
...
publicRouter.post("/events/:id/applications", limited, async (req, res) => {
  try {
    await ensureSchema();
    const b = req.body || {},
    ...
    if (
      ...
      age > 100 ||
      !b.consent ||
      !clean(b.eligibilityResponse, 40)
    )
      return res
        .status(400)
        .json({ error: "Complete all required application fields." });
    const event = (
    ...
    if (
      ...
      new Date(event.event_start_at) <= new Date()
    )
      return res
        .status(409)
        .json({ error: "Applications for this event are closed." });
    if (
    ...
    const seq = await pool.query(
      ...
      number = `EV-${new Date().getFullYear()}-${String(seq.rows[0].value).padStart(6, "0")}`,
      id = randomUUID();
    await pool.query(
      `INSERT INTO event_applications(id,application_number,event_id,full_name,age,eligibility_response,email,phone,status,registration_status,attendance_status,customer_language,consent_version,consent_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'pending','pending','unknown',$9,'event-v1',NOW())`,
      [
        id,
        number,
        event.id,
    ...
    void sendEmail({
      to: email,
      subject: "We received your event application",
      html: emailShell(
        `<h1>Your application has been received</h1><p><strong>${escapeHtml(number)}</strong> is currently pending. Submission does not guarantee a place.</p><p>Aida will review it and contact you when your place is confirmed.</p>`,
      ),
    }).catch(() => {});
    void sendEmail({
      to: process.env.EVENT_NOTIFICATION_EMAIL || OWNER_EMAIL,
      subject: `New event applicant — ${number}`,
      html: emailShell(
        `<h1>New event application</h1><p>${escapeHtml(fullName)} · age ${age} · ${escapeHtml(email)} · ${escapeHtml(phone)}</p><p>Event: ${escapeHtml(event.title_en)}</p>`,
      ),
    }).catch(() => {});
    return res
      .status(201)
...
adminCheckoutRouter.get("/orders/:id/receipt", async (req, res) => {
  const result = await pool.query(
    "SELECT receipt_storage_key,receipt_original_name,receipt_mime_type FROM checkout_orders WHERE id=$1",
    [req.params.id],
  );
  ...
  res
    ...
    .setHeader(
      "Content-Disposition",
      `inline; filename*=UTF-8''${encodeURIComponent(row.receipt_original_name)}`,
    );
  return res.send(
    await readFile(path.join(receiptDir, row.receipt_storage_key)),
  );
});
adminCheckoutRouter.patch("/orders/:id/status", async (req, res) => {
  const next = clean(req.body?.status, 20);
  if (
    !["pending", "packaging", "shipped", "completed", "cancelled"].includes(
      next,
    )
  )
  ...
  if (previous === next)
    return res.json({ order: current.rows[0], emailSent: false });
  const result = await pool.query(
    `UPDATE checkout_orders SET status=$1,tracking_carrier=$2,tracking_number=$3,tracking_url=$4,internal_note=$5,updated_at=NOW(),packaging_at=CASE WHEN $1='packaging' THEN NOW() ELSE packaging_at END,shipped_at=CASE WHEN $1='shipped' THEN NOW() ELSE shipped_at END,completed_at=CASE WHEN $1='completed' THEN NOW() ELSE completed_at END,cancelled_at=CASE WHEN $1='cancelled' THEN NOW() ELSE cancelled_at END WHERE id=$6 RETURNING *`,
    [
      next,
      clean(req.body.trackingCarrier, 100) || null,
      clean(req.body.trackingNumber, 100) || null,
      clean(req.body.trackingUrl, 500) || null,
      clean(req.body.internalNote, 1000) || null,
      req.params.id,
    ],
  );
  await pool.query(
    "INSERT INTO checkout_order_status_history(id,order_id,previous_status,new_status,changed_by_admin_id,internal_note) VALUES($1,$2,$3,$4,'admin',$5)",
    [
      randomUUID(),
      req.params.id,
      previous,
  ...
  const subjects: any = {
    packaging: "Your order is being prepared",
    shipped: "Your order is on its way",
    completed: "Your order is complete",
    cancelled: "An update about your order",
  };
  if (subjects[next])
    void sendEmail({
      to: order.customer_email,
      subject: `${subjects[next]} — ${order.order_number}`,
      html: emailShell(
        `<h1>${escapeHtml(subjects[next])}</h1><p>Order <strong>${escapeHtml(order.order_number)}</strong> is now ${escapeHtml(next)}.</p>${next === "shipped" && order.tracking_number ? `<p>Tracking: ${escapeHtml(order.tracking_carrier || "")} ${escapeHtml(order.tracking_number)}</p>` : ""}`,
      ),
    }).catch(() => {});
  return res.json({ order, emailSent: Boolean(subjects[next]) });
});
adminCheckoutRouter.get("/bank", async (_req, res) => {
  await ensureSchema();
  res.json({
    accounts: (
      await pool.query("SELECT * FROM bank_transfer_settings ORDER BY currency")
    ).rows,
  });
});
adminCheckoutRouter.get("/event-applications", async (_req, res) => {
  await ensureSchema();
  const result = await pool.query(
    "SELECT * FROM event_applications ORDER BY created_at DESC",
  );
  res.json({ applications: result.rows });
});
adminCheckoutRouter.patch(
  "/event-applications/:id/status",
  async (req, res) => {
    await ensureSchema();
    const next = clean(req.body?.status, 20);
    if (
    ...
    const current = (
      await pool.query(
        "SELECT a.*,e.total_capacity,e.title_en FROM event_applications a JOIN event_banner_config e ON e.id=a.event_id WHERE a.id=$1",
        [req.params.id],
      )
    ).rows[0];
    if (!current)
      return res.status(404).json({ error: "Application not found" });
    if (next === "accepted" && current.status !== "accepted") {
      const accepted = Number(
        (
          await pool.query(
            "SELECT COUNT(*) count FROM event_applications WHERE event_id=$1 AND status IN ('accepted','attended')",
            [current.event_id],
          )
        ).rows[0].count,
      );
      const legacy = Number(
        (
          await pool.query(
            "SELECT COALESCE(SUM(seat_count),0) count FROM newsletter_event_interests WHERE campaign_id=$1 AND reservation_status IN ('confirmed','attended')",
            [current.event_id],
          )
        ).rows[0].count,
      );
    ...
    const result = await pool.query(
      ...
      [next, clean(req.body?.adminNote, 1000) || null, req.params.id],
    );
    const application = result.rows[0];
    if (
      application.status !== current.status &&
      ["accepted", "waitlisted", "rejected"].includes(next)
    )
      void sendEmail({
        to: application.email,
        subject: `Your event application is ${next}`,
        html: emailShell(
          `<h1>Application ${escapeHtml(next)}</h1><p>Your application <strong>${escapeHtml(application.application_number)}</strong> for ${escapeHtml(current.title_en)} is now ${escapeHtml(next)}.</p>`,
        ),
      }).catch(() => {});
    return res.json({ application });
  },
);
adminCheckoutRouter.put("/bank/:currency", async (req, res) => {
  const currency = req.params.currency === "USD" ? "USD" : "TRY";
  await ensureSchema();
  const b = req.body || {};
  const result = await pool.query(
    `UPDATE bank_transfer_settings SET enabled=$1,account_holder=$2,bank_name=$3,iban=$4,swift_bic=$5,branch_info=$6,bank_address=$7,instructions=$8,notification_email=$9,updated_at=NOW() WHERE currency=$10 RETURNING *`,
    [
      Boolean(b.enabled),
      clean(b.accountHolder, 160),
      clean(b.bankName, 160),
      clean(b.iban, 80),
      clean(b.swiftBic, 40) || null,
      clean(b.branchInfo, 200) || null,
      clean(b.bankAddress, 300) || null,
      clean(b.instructions, 1000) || null,
      clean(b.notificationEmail, 254) || null,
      currency,
    ],
  );
