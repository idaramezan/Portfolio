  ...
  return rows
    .flatMap((raw: any) => {
      if (
        ...
        return [];
      const externalUrl = productUrl(raw.slug);
      if (!externalUrl) return [];
      const variants: InternationalProduct["variants"] = (
        ...
        return [
          {
            ...
            price: { amount, currency, formatted: money(amount, currency) },
          },
        ];
      });
      if (!variants.length) return [];
      const lowest = variants.reduce((a, b) =>
    ...
    .sort(
      (a: InternationalProduct, b: InternationalProduct) =>
...
async function refresh() {
  ...
  try {
    ...
    if (cache) {
      cache.lastError = "Refresh failed";
      return cache;
    }
    throw error;
  } finally {
...
router.get("/international", async (_req, res) => {
  ...
  try {
    const data =
      cache && cache.expiresAt > Date.now() ? cache : await refresh();
    return res.json({
      enabled,
      products: data.products,
      fetchedAt: data.fetchedAt,
      collection: data.collection,
      shopUrl: configuredShopUrl()?.toString() || null,
      stale: data.expiresAt <= Date.now(),
    });
  } catch {
    return res.status(503).json({
      ...
      products: [],
      shopUrl: configuredShopUrl()?.toString() || null,
      message: "The international collection is temporarily unavailable here.",
    });
  }
});
router.get("/admin/international/status", (_req, res) =>
  res.json({
    enabled: process.env.FOURTHWALL_INTEGRATION_ENABLED !== "false",
    tokenConfigured: Boolean(process.env.FOURTHWALL_STOREFRONT_TOKEN),
    shopUrl: configuredShopUrl()?.toString() || null,
    collection: process.env.FOURTHWALL_COLLECTION_HANDLE || "all",
    productCount: cache?.products.length || 0,
    lastSuccessfulSync: cache?.fetchedAt || null,
...
router.post("/admin/international/refresh", async (_req, res) => {
  try {
    cache = null;
    ...
    return res.status(503).json({
      ok: false,
      error: "International products could not be refreshed",
    });
  }
});
router.delete("/admin/international/cache", (_req, res) => {
  cache = null;
