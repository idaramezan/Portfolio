...
async function renderCampaignBlocks(blocks: CampaignBlock[]) {
  ...
  const responsiveStyles = `<style>@media only screen and (max-width:620px){.email-row,.email-row tbody,.email-row tr{display:block!important;width:100%!important}.email-column{display:block!important;width:100%!important;box-sizing:border-box!important;padding:12px!important}h1{font-size:30px!important;line-height:1.12!important}h2{font-size:25px!important}}</style>`;
  const rendered = (
    await Promise.all(
      blocks.map(async (block) => {
        ...
        if (block.type === "divider")
          return '<hr style="border:0;border-top:1px dashed #beb3a4;margin:26px 0">';
        ...
        if (block.type === "button") {
          ...
          return `<p style="margin:24px 0;text-align:center"><a href="${escapeHtml(url)}" style="display:inline-block;background:#c15a83;color:#fff9f1;padding:14px 23px;font-family:Arial Black,Arial,sans-serif;text-decoration:none;font-weight:900">${escapeHtml(block.text.trim())}</a></p>`;
        }
        if (block.type === "text") {
          ...
          const linked =
            link && linkText
              ? `${text}<br><a href="${escapeHtml(link)}" style="color:#c15a83;text-decoration:underline">${escapeHtml(linkText)}</a>`
              : text;
...
function buildPaintingEventInterestEmail(input: {
  ...
  const participationHtml = `<p style="margin:0 0 5px;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#c15a83">Participation fee</p><p style="margin:0;font-size:28px;font-weight:700">${escapeHtml(String(config.participation_price_try))} TL</p>`;
  const detailRows = [
    ["Date and time", date],
    ...
    .map(
      ([label, value]) =>
        `<tr><th scope="row" style="padding:9px 12px 9px 0;border-bottom:1px solid #ded2c6;text-align:left;vertical-align:top;font-size:13px;color:#756b70">${escapeHtml(label)}</th><td style="padding:9px 0;border-bottom:1px solid #ded2c6;text-align:left;vertical-align:top;font-size:14px;font-weight:600">${escapeHtml(value)}</td></tr>`,
    )
    .join("");
  const content = `<h1 style="margin:0 0 22px;font-size:31px;line-height:1.2">${escapeHtml(config.title_en)}</h1><p style="font-size:16px;line-height:1.7">${escapeHtml(config.description_en)}</p><table role="presentation" style="width:100%;margin:24px 0;border-collapse:collapse">${detailRows}</table><div style="margin:24px 0;padding:18px;border-left:4px solid #c15a83;background:#f7e5ec">${participationHtml}</div><p style="font-size:16px;line-height:1.7"><strong>Your email has registered your interest, but it has not reserved a place.</strong></p><p style="font-size:16px;line-height:1.7">Your attendance is confirmed personally by Aida on WhatsApp.</p><p style="margin:26px 0;text-align:center"><a href="${escapeHtml(input.whatsappUrl)}" style="display:inline-block;background:#c15a83;color:#fff9f1;padding:14px 22px;font-family:Arial Black,Arial,sans-serif;text-decoration:none;font-weight:900">Contact Aida to reserve my place</a></p>`;
